...
/* ── GMAttackPanel — ГМ видит все активные сцены боя ── */
function GMAttackPanel(pr){
  var attacks=pr.attacks||{};
  var addLog=pr.addLog;var onRoll=pr.onRoll;var characters=pr.characters||[];
  /* Показываем ВСЕ сцены NPC-атак: и ожидающие уклонения, и готовые к урону */
  ...
  return(<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.65)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:997,animation:"fadeIn 0.2s"}}>
    <div style={{background:"#fefcf5",border:"3px solid #c4b8a4",borderRadius:16,padding:"16px 20px",minWidth:280,maxWidth:370,maxHeight:"82vh",overflowY:"auto",boxShadow:"0 20px 60px rgba(0,0,0,0.4)",animation:"popIn 0.3s",display:"flex",flexDirection:"column",gap:8}}>
      <div style={{fontFamily:"'Cinzel',serif",fontWeight:900,fontSize:15,textAlign:"center",marginBottom:2}}>⚔️ Сцены боя</div>
      {active.map(function(entry){
        var id=entry[0];var atk=entry[1];
        var waitingDodge=atk.status==="pending_dodge";
        var dodged=!waitingDodge&&atk.dodgeRoll>=atk.hitRoll;
        var tgtChar=characters.find(function(x){return x._fbId===atk.targetId;});
        function doRollDmg(){
          if(!tgtChar)return;
          var m=(atk.dmgDice||"1d6").match(/(\d+)d(\d+)/);
          if(!m)return;
          var dice=rN(parseInt(m[1]),parseInt(m[2]));
          var rawDmg=sm(dice)+(atk.dmgBonus||0);
          var zoneObj=ZONES.find(function(z){return z.name===atk.zone;})||ZONES[2];
          ...
          var ae=zoneObj.ignoreArmor?{ad:0,hd:multiplied,desc:"🔓 "+zoneObj.name+" ×"+zoneObj.mult+" игнор брони → HP−"+multiplied}:calcAE(pArmorType,atk.dmgType||"Р",multiplied);
          ...
          addLog({who:atk.attackerName,type:"dmg_npc",label:"💥 "+atk.weaponName+" → "+atk.targetName+" ["+zoneObj.e+atk.zone+"] "+ae.hd+" HP",detail:ae.desc+" | ❤️ "+pCur+"→"+newHp,total:ae.hd});
          onRoll({label:atk.attackerName+" 💥 "+atk.weaponName+" Урон",d10:null,parts:[{label:atk.dmgDice||"1d6",value:sm(dice)},{label:"Бнс",value:atk.dmgBonus||0}],total:rawDmg,subtext:ae.desc+"\n→ "+atk.targetName+": "+pCur+"→"+newHp+" HP"});
          remove(ref(db,"rooms/"+pr.room+"/pendingAttacks/"+id));
        }
        function skipDmg(){remove(ref(db,"rooms/"+pr.room+"/pendingAttacks/"+id));}
        return(<div key={id} style={{background:"#fff",borderRadius:10,padding:"10px 12px",border:"2px solid "+(waitingDodge?"#f59e0b40":dodged?"#10b98130":"#ef444430")}}>
          {/* Заголовок сцены */}
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
            <span style={{fontWeight:700,fontSize:12}}>{atk.attackerName+" → "+atk.targetName}</span>
            <span style={{fontSize:9,padding:"2px 6px",borderRadius:99,fontWeight:700,background:waitingDodge?"#fef3c7":dodged?"#d1fae5":"#fee2e2",color:waitingDodge?"#92400e":dodged?"#065f46":"#991b1b"}}>{waitingDodge?"⏳ ждём уклонения":dodged?"✅ уклонился":"❌ попало"}</span>
          </div>
          {/* Оружие и зона */}
          <div style={{fontSize:9,color:"#8b7e6a",marginBottom:8,textAlign:"center"}}>{atk.weaponName+" · "+atk.dmgDice+" "+atk.dmgType+" · "+(atk.zone||"Торс")}</div>
          {/* Броски — атака всегда, уклонение если есть */}
          <div style={{display:"flex",gap:8,justifyContent:"center",alignItems:"center",marginBottom:8}}>
            <div style={{textAlign:"center",background:"#fef2f2",borderRadius:8,padding:"6px 14px"}}>
              <div style={{fontSize:8,color:"#6b7280",marginBottom:1}}>Атака</div>
              <div style={{fontFamily:"'Cinzel',serif",fontWeight:900,fontSize:26,color:"#ef4444"}}>{atk.hitRoll}</div>
            </div>
            <div style={{fontSize:16,color:"#8b7e6a",fontWeight:700}}>vs</div>
            <div style={{textAlign:"center",background:waitingDodge?"#fefdfb":"#f0fdf4",borderRadius:8,padding:"6px 14px",border:waitingDodge?"1px dashed #d1d5db":"none"}}>
              <div style={{fontSize:8,color:"#6b7280",marginBottom:1}}>Уклонение</div>
              {waitingDodge
                ?<div style={{fontSize:13,color:"#9ca3af",fontStyle:"italic",padding:"6px 0"}}>ждём...</div>
                :<div style={{fontFamily:"'Cinzel',serif",fontWeight:900,fontSize:26,color:"#10b981"}}>{atk.dodgeRoll||0}</div>
              }
            </div>
          </div>
          {/* Кнопки действий */}
          {waitingDodge&&<div style={{fontSize:9,color:"#8b7e6a",textAlign:"center",fontStyle:"italic"}}>Игрок бросает уклонение...</div>}
          {!waitingDodge&&(dodged
            ?<button onClick={skipDmg} style={{width:"100%",padding:7,borderRadius:7,border:"none",background:"#10b981",color:"#fff",fontWeight:700,fontSize:11,cursor:"pointer"}}>✅ Уклонился — закрыть</button>
            :<button onClick={doRollDmg} style={{width:"100%",padding:9,borderRadius:7,border:"none",background:"#ef4444",color:"#fff",fontFamily:"'Cinzel',serif",fontWeight:900,fontSize:14,cursor:"pointer"}}>💥 Бросить урон!</button>
          )}
        </div>)
      })}
    </div>
...
/* ── PlayerAttackNotif — ГМ кидает уклонение за NPC когда игрок атакует ── */
function PlayerAttackNotif(pr){
  ...
  var mine=Object.entries(attacks).filter(function(e){return e[1].fromPlayer&&e[1].status==="pending_dodge";});
  ...
  return(<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.7)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:998,animation:"fadeIn 0.2s"}}>
    <div style={{background:"linear-gradient(135deg,#eff6ff,#dbeafe)",border:"3px solid #3b82f6",borderRadius:16,padding:"18px 22px",textAlign:"center",minWidth:260,maxWidth:340,boxShadow:"0 20px 60px rgba(0,0,0,0.5)",animation:"popIn 0.3s"}}>
      {mine.length>1&&<div style={{fontSize:9,color:"#8b7e6a",marginBottom:4}}>{"Атака 1 из "+mine.length}</div>}
      ...
      <div style={{background:"#fff",borderRadius:10,padding:"8px 12px",marginBottom:10}}>
        <div style={{fontSize:9,color:"#6b7280",marginBottom:2}}>Бросок на попадание</div>
        <div style={{fontFamily:"'Cinzel',serif",fontWeight:900,fontSize:36,color:"#1d4ed8"}}>{atk.hitRoll}</div>
        {atk.weaponName&&<div style={{fontSize:9,color:"#8b7e6a",marginTop:2}}>{atk.weaponName+" · "+atk.dmgType+" · "+atk.zone}</div>}
      </div>
      <div style={{fontSize:10,color:"#5c5548",marginBottom:10}}>Уклонение за {atk.npcName}: d10 + DEX + Dodge</div>
      ...
      <button onClick={acceptHit} style={{width:"100%",padding:6,borderRadius:7,border:"2px solid #3b82f640",background:"none",color:"#3b82f6",fontWeight:700,fontSize:10,cursor:"pointer"}}>Принять удар</button>
    </div>
  </div>)}
...
if(tgtNpc&&tgtId&&pr.savePendingAttack){pr.savePendingAttack({id:"atk_"+Date.now(),fromPlayer:true,attackerName:c.name||"???",npcId:tgtId,npcName:tgtNpc.name,hitRoll:t,weaponName:w.name,dmgDice:w.dmgDice||"1d6",dmgType:w.dmgType||"Р",dmgBonus:w.bonus||0,zone:selZone,status:"pending_dodge",ts:Date.now()});}
...
function npcAttack(s,w){
  ...
  if(playerTgtId&&pr.savePendingAttack){
    pr.savePendingAttack({id:"atk_"+Date.now(),attackerName:s.name,targetId:playerTgtId,targetName:tgtName,hitRoll:t,weaponName:w.name,dmgDice:w.dice||"1d6",dmgType:w.dmgType||"Р",dmgBonus:w.bonus||0,zone:playerZone,status:"pending_dodge",ts:Date.now()});
  }
}
