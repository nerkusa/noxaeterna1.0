...
/* ── PlayerAttackStatus — показывается САМОМУ игроку пока его атака обрабатывается ── */
function PlayerAttackStatus(pr){
  var attacks=pr.attacks||{};
  var myName=pr.myName;var myId=pr.myId;
  ...
  var mine=Object.entries(attacks).filter(function(e){
    return e[1].fromPlayer&&(e[1].attackerId===myId||(e[1].attackerName&&e[1].attackerName===myName))&&(e[1].status==="pending_dodge"||e[1].status==="pending_dmg");
  });
  ...
  return(<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.72)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:997,animation:"fadeIn 0.2s"}}>
    <div style={{background:"#fefcf5",border:"3px solid "+(waiting?"#f59e0b":dodged?"#10b981":"#ef4444"),borderRadius:16,padding:"18px 22px",textAlign:"center",minWidth:270,maxWidth:350,boxShadow:"0 20px 60px rgba(0,0,0,0.5)",animation:"popIn 0.3s"}}>
      ...
      {/* Уклонение NPC */}
      <div style={{background:"#fff",border:"1px solid "+(waiting?"#d1d5db":"#10b98140"),borderRadius:10,padding:"8px 12px"}}>
        ...
        {waiting
          ...
          :<div>
            ...
            {!dodged&&<div style={{fontSize:10,color:"#8b7e6a",marginTop:4,fontStyle:"italic"}}>ГМ бросает урон...</div>}
          </div>
        }
      </div>
    </div>
...
function PendingAttackPopup(pr){
  ...
  var pending=Object.entries(attacks).filter(function(e){
    return e[1].targetId===myId&&(e[1].status==="pending_dodge"||e[1].status==="pending_dmg"||!e[1].status);
  });
  if(pending.length===0)return null;
  var entry=pending[0];var id=entry[0];var atk=entry[1];
  var inf=cF(myChar);var fs=inf.fs;var es=inf.eSk;
  var waiting=atk.status==="pending_dodge"||!atk.status;
  var dodged=!waiting&&(atk.dodgeRoll||0)>=(atk.hitRoll||0);
  var atkD=atk.atkD||"?";var atkREF=atk.atkREF||0;var atkSkill=atk.atkSkill||0;
  var atkBonus=atk.atkBonus||0;var atkSkillName=atk.atkSkillName||"Навык";
  var dodgeDetail=atk.dodgeDetail||"";
  function doDodge(){
    ...
    update(ref(db,"rooms/"+pr.room+"/pendingAttacks/"+id),{dodgeRoll:t,dodgeDetail:"d10("+d+")+DEX("+dv+")+Dodge("+dg+")="+t,status:dodgedNow?"done":"pending_dmg"});
    addLog({who:myChar.name||"???",type:"dodge",label:(dodgedNow?"✅ Уклонился от ":"❌ Не уклонился от ")+atk.attackerName,detail:"🎲"+d+" + DEX("+dv+") + Dodge("+dg+") = "+t+" vs "+atk.hitRoll,total:t});
    onRoll({label:"🛡️ Уклонение",d10:d,parts:[{label:"DEX",value:dv},{label:"Dodge",value:dg}],total:t,subtext:"Атака: "+atk.hitRoll+" | Уклонение: "+t+(dodgedNow?" ✅ УКЛОНИЛСЯ":" ❌ ПОПАЛО")});
  }
  function acceptHit(){
  ...
  return(<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.75)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:998,animation:"fadeIn 0.2s"}}>
    <div style={{background:"#fefcf5",border:"3px solid "+(waiting?"#ef4444":dodged?"#10b981":"#ef4444"),borderRadius:16,padding:"18px 22px",textAlign:"center",minWidth:270,maxWidth:350,boxShadow:"0 20px 60px rgba(0,0,0,0.5)",animation:"popIn 0.3s"}}>
      ...
      <div style={{background:"#fff",border:"1px solid "+(waiting?"#d1d5db":"#10b98140"),borderRadius:10,padding:"8px 12px",marginBottom:waiting?10:0}}>
        ...
        {waiting
          ...
          :<div>
            ...
            {!dodged&&(function(){
              var sh=myChar.shield;var shHp=myChar.shieldHp||0;
              var SHIELDS=[{id:"none"},{id:"light",name:"Лёгкий",absorb:0.5},{id:"medium",name:"Средний",absorb:0.75},{id:"tower",name:"Башенный",absorb:1}];
              var shObj=SHIELDS.find(function(s){return s.id===sh});
              ...
              var canShield=shObj&&sh&&sh!=="none"&&shHp>0&&!is2h&&!atk.shieldUsed;
              if(!canShield)return <div style={{fontSize:10,color:"#8b7e6a",marginTop:6,fontStyle:"italic"}}>{sh&&sh!=="none"&&shHp<=0?"💔 Щит сломан":"ГМ наносит урон..."}</div>;
              return <div style={{marginTop:8}}>
                <button onClick={function(){update(ref(db,"rooms/"+pr.room+"/pendingAttacks/"+id),{shieldUsed:true,shieldAbsorb:shObj.absorb,shieldName:shObj.name});}} style={{width:"100%",padding:9,borderRadius:8,border:"none",background:"#0369a1",color:"#fff",fontFamily:"'Cinzel',serif",fontWeight:900,fontSize:13,cursor:"pointer",marginBottom:4}}>🛡 Выставить {shObj.name} щит ({shObj.absorb*100}% поглощения)</button>
                <div style={{fontSize:9,color:"#8b7e6a",textAlign:"center"}}>HP щита: {shHp} · или ждите урон от ГМ</div>
              </div>;
            })()}
          </div>
        }
...
function GMAttackPanel(pr){
  ...
  var active=Object.entries(attacks).filter(function(e){
    return !e[1].fromPlayer&&(e[1].status==="pending_dodge"||e[1].status==="pending_dmg"||(!e[1].status&&!e[1].fromPlayer&&e[1].targetId));
  });
  ...
  function doRollDmg(){
    ...
    if(atk.shieldUsed&&atk.shieldAbsorb){
      ...
      var newShHp=Math.max(0,(tgtChar.shieldHp||0)-absorbed);
      var updShield={shieldHp:newShHp};
      if(newShHp<=0)updShield.shield="none";
      Object.assign(tgtChar,updShield);
    }
    ...
    if(atk.shieldUsed){updP.shieldHp=Math.max(0,(tgtChar.shieldHp||0)-shieldDmg);if(updP.shieldHp<=0)updP.shield="none";}
...
function PlayerAttackNotif(pr){
  ...
  if(needDodge.length===0&&needDmg.length>0){
    ...
    return(<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.65)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:998,animation:"fadeIn 0.2s"}}>
      <div style={{background:"#fefcf5",border:"3px solid "+(dodgedNpc?"#10b981":"#ef4444"),borderRadius:16,padding:"18px 22px",textAlign:"center",minWidth:270,maxWidth:350,boxShadow:"0 20px 60px rgba(0,0,0,0.5)",animation:"popIn 0.3s"}}>
        <div style={{display:"flex",justifyContent:"flex-end",marginBottom:2}}><button onClick={skipNpcDmg} style={{background:"none",border:"none",fontSize:18,cursor:"pointer",color:"#8b7e6a"}}>✕</button></div>
        ...
        <div style={{display:"flex",gap:8,justifyContent:"center",marginBottom:10}}>
          <div style={{textAlign:"center",background:"#fef2f2",borderRadius:8,padding:"6px 14px"}}><div style={{fontSize:8,color:"#6b7280",marginBottom:1}}>Атака</div><div style={{fontFamily:"'Cinzel',serif",fontWeight:900,fontSize:26,color:"#ef4444"}}>{dmgAtk.hitRoll}</div></div>
          <div style={{fontSize:15,color:"#8b7e6a",fontWeight:700,alignSelf:"center"}}>vs</div>
          <div style={{textAlign:"center",background:"#f0fdf4",borderRadius:8,padding:"6px 14px"}}><div style={{fontSize:8,color:"#6b7280",marginBottom:1}}>Уклонение</div><div style={{fontFamily:"'Cinzel',serif",fontWeight:900,fontSize:26,color:"#10b981"}}>{dmgAtk.dodgeRoll||0}</div></div>
        </div>
        ...
        {dodgedNpc
          ...
          :<button onClick={doNpcDmg} style={{width:"100%",padding:10,borderRadius:8,border:"none",background:"#ef4444",color:"#fff",fontFamily:"'Cinzel',serif",fontWeight:900,fontSize:14,cursor:"pointer"}}>💥 Бросить урон + зону!</button>
        }
      </div>
    </div>);
  }
...
var SHIELDS=[{id:"none",name:"Нет",absorb:0,hp:0,bodyReq:0},{id:"light",name:"Лёгкий щит",absorb:0.5,hp:15,bodyReq:4},{id:"medium",name:"Средний щит",absorb:0.75,hp:25,bodyReq:6},{id:"tower",name:"Башенный щит",absorb:1,hp:40,bodyReq:8}];
var curShield=SHIELDS.find(function(s){return s.id===(c.shield||"none")})||SHIELDS[0];
var shHp=c.shieldHp||0;var shMaxHp=c.shieldMaxHp||0;
var equippedW=(c.weapons||[]).find(function(w){return w.id===c.equippedWeapon});
var shieldBlocked=equippedW&&equippedW.hands===2;
...
{/* Слот щита */}
<div style={{marginBottom:6,padding:"5px 7px",background:"#f0f9ff",border:"1px solid #38bdf828",borderRadius:7}}>
<div style={{fontSize:8,fontWeight:700,color:"#0369a1",marginBottom:4}}>🛡 Щит (левая рука)</div>
{shieldBlocked&&<div style={{fontSize:8,color:"#f59e0b",marginBottom:4}}>⚠️ Двуручное оружие — щит недоступен</div>}
<div style={{display:"flex",gap:3,flexWrap:"wrap",marginBottom:curShield.id!=="none"?5:0}}>
{SHIELDS.map(function(s){var ok=fs.BODY>=s.bodyReq;var sel=c.shield===s.id||(s.id==="none"&&!c.shield);return(<button key={s.id} disabled={shieldBlocked&&s.id!=="none"||(!ok&&s.id!=="none")} onClick={function(){if(shieldBlocked&&s.id!=="none")return;if(!ok&&s.id!=="none"){alert("BODY("+fs.BODY+") < "+s.bodyReq);return;}sv(Object.assign({},c,{shield:s.id,shieldHp:sel?shHp:s.hp,shieldMaxHp:s.hp}));}} style={{padding:"2px 7px",borderRadius:5,border:sel?"2px solid #0369a1":"1px solid #e8e0d4",background:sel?"#e0f2fe":"#fefdfb",fontSize:8,fontWeight:sel?700:400,cursor:shieldBlocked||(!ok&&s.id!=="none")?"not-allowed":"pointer",color:sel?"#0c4a6e":ok?"#2d2a24":"#9ca3af",opacity:!ok&&s.id!=="none"?0.5:1}}>{s.name}</button>);})}
</div>
{curShield.id!=="none"&&<div>
<div style={{display:"flex",alignItems:"center",gap:4}}><span style={{fontSize:8,color:"#0369a1",flex:1}}>{"HP: "+shHp+"/"+shMaxHp+" · поглощает "+(curShield.absorb*100)+"%"}</span><button onClick={function(){sv(Object.assign({},c,{shieldHp:Math.max(0,shHp-1)}))}} style={Object.assign({},S.sm,{width:14,height:14,fontSize:8})}>−</button><button onClick={function(){sv(Object.assign({},c,{shieldHp:Math.min(shMaxHp,shHp+1)}))}} style={Object.assign({},S.sm,{width:14,height:14,fontSize:8})}>+</button></div>
<div style={{background:"#fff",borderRadius:3,height:5,overflow:"hidden",marginTop:3}}><div style={{height:"100%",width:(shMaxHp>0?(shHp/shMaxHp)*100:0)+"%",background:shHp<=0?"#ef4444":"#0369a1",borderRadius:3}}/></div>
{shHp<=0&&<div style={{fontSize:8,color:"#ef4444",fontWeight:700,marginTop:2}}>💔 Щит сломан!</div>}
</div>}
</div>
...
var _wdi2=useState("2d6");var wdi2=_wdi2[0];var sWDI2=_wdi2[1];
/* weapon edit state */
...
<div style={{display:"flex",gap:3}}>{[1,1.5,2].map(function(h){return<button key={h} onClick={function(){sWH(h)}} style={{flex:1,padding:"3px 0",borderRadius:5,border:wh===h?"2px solid #3b82f6":"1px solid #e8e0d4",background:wh===h?"#eff6ff":"#fefdfb",fontSize:8,fontWeight:wh===h?700:400,cursor:"pointer",color:wh===h?"#1d4ed8":"#5c5548"}}>{h===1?"Одноручное":h===1.5?"Полуторное":"Двуручное"}</button>})}</div>
{wh!==1.5
...
<div style={{display:"flex",gap:3,alignItems:"center"}}><span style={{fontSize:8,color:"#8b7e6a",width:42}}>2 руки:</span><input style={Object.assign({},S.inp,{flex:1,fontSize:9,padding:3})} value={wdi2} onChange={function(e){sWDI2(e.target.value)}} placeholder="2d8"/></div>
</div>}
<button onClick={function(){if(!wn.trim())return;var newW={id:Date.now(),name:wn.trim(),type:wt,dmgType:wdt,bonus:wb,dmgDice:wdi,hands:wh};if(wh===1.5)newW.dmgDice2h=wdi2;sv(Object.assign({},c,{weapons:(c.weapons||[]).concat([newW])}));sWN("");sSA(false);sWH(1);sWDI("1d6");sWDI2("2d6");}} style={{padding:5,borderRadius:5,border:"none",background:"#10b981",color:"#fff",fontWeight:700,fontSize:10,cursor:"pointer"}}>Добавить</button>
</div>}
{(c.weapons||[]).map(function(w){var sk=WS[w.type]||"Simple Weapon";var isEq=c.equippedWeapon===w.id;var curMode=c.weaponMode||"1h";var activeDice=(w.hands===1.5&&curMode==="2h")?(w.dmgDice2h||w.dmgDice):w.dmgDice;var handsLabel=w.hands===2?"двуручное":w.hands===1.5?"полуторное":"одноручное";var handsClr=w.hands===2?"#991b1b":w.hands===1.5?"#92400e":"#1e40af";var handsBg=w.hands===2?"#fee2e2":w.hands===1.5?"#fef3c7":"#eff6ff";
...
<button onClick={function(){var newEq=isEq?null:w.id;var upd={equippedWeapon:newEq};if(w.hands===2&&c.shield&&c.shield!=="none"){upd.shield="none";}sv(Object.assign({},c,upd));}} style={{fontSize:7,padding:"2px 6px",borderRadius:3,border:isEq?"1px solid #10b98140":"1px solid #e8e0d4",background:isEq?"#d1fae5":"#fefdfb",cursor:"pointer",color:isEq?"#065f46":"#5c5548",fontWeight:700}}>{isEq?"✓ Снаряжено":"Снарядить"}</button>
<button onClick={function(){sv(Object.assign({},c,{weapons:(c.weapons||[]).filter(function(x){return x.id!==w.id}),equippedWeapon:c.equippedWeapon===w.id?null:c.equippedWeapon}))}} style={{background:"none",border:"none",color:"#ef4444",fontSize:10,cursor:"pointer"}}>✕</button>
</div></div>
{w.hands===1.5&&<div style={{display:"flex",gap:3,marginBottom:3}}>
<button onClick={function(){sv(Object.assign({},c,{weaponMode:"1h"}))}} style={{flex:1,padding:"2px 0",borderRadius:4,border:curMode==="1h"?"2px solid #3b82f6":"1px solid #e8e0d4",background:curMode==="1h"?"#eff6ff":"#fefdfb",fontSize:8,fontWeight:curMode==="1h"?700:400,cursor:"pointer",color:curMode==="1h"?"#1d4ed8":"#5c5548"}}>1 рука · {w.dmgDice}</button>
<button onClick={function(){var upd={weaponMode:"2h"};if(c.shield&&c.shield!=="none")upd.shield="none";sv(Object.assign({},c,upd));}} style={{flex:1,padding:"2px 0",borderRadius:4,border:curMode==="2h"?"2px solid #3b82f6":"1px solid #e8e0d4",background:curMode==="2h"?"#eff6ff":"#fefdfb",fontSize:8,fontWeight:curMode==="2h"?700:400,cursor:"pointer",color:curMode==="2h"?"#1d4ed8":"#5c5548"}}>2 руки · {w.dmgDice2h||w.dmgDice}</button>
</div>}
<div style={{display:"flex",gap:3}}>
...
<button onClick={function(){
  ...
  oR({label:w.name+" 💥 Урон",d10:null,parts:[{label:activeDice,value:sm(dice)},{label:"Бнс",value:(w.bonus||0)+warDmgBon}],total:rawDmg,subtext:"Тип: "+w.dmgType+(warDmgBon?" ⚔️+5":"")+(tgtNpc?" → "+tgtNpc.name+"\nЗона: "+selZone:" (нет цели)")});
