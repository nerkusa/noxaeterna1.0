...
/* ── MapView ── */
function MapView(pr){var md=pr.mapData||{};var sMD=pr.saveMap;var chars=pr.characters||[];var isGM=pr.isGM;var cId=pr.charId;var img=md.image||"";var tokens=md.tokens||[];var _dr=useState(null);var drId=_dr[0];var sDrId=_dr[1];var _off=useState({x:0,y:0});var drOff=_off[0];var sDrOff=_off[1];var _sn=useState(false);var sn=_sn[0];var sSN=_sn[1];var _nn=useState("");var nn=_nn[0];var sNN=_nn[1];var _nc=useState("#ef4444");var nc=_nc[0];var sNC=_nc[1];var _ns=useState(14);var ns=_ns[0];var sNS=_ns[1];
...
function startDr(e,t){if(!canDr(t))return;e.preventDefault();var r=e.currentTarget.parentElement.getBoundingClientRect();var cx=e.touches?e.touches[0].clientX:e.clientX;var cy=e.touches?e.touches[0].clientY:e.clientY;sDrId(t.id);sDrOff({x:((cx-r.left)/r.width)*100-t.x,y:((cy-r.top)/r.height)*100-t.y})}
function onMv(e){if(!drId)return;e.preventDefault();var r=e.currentTarget.getBoundingClientRect();var cx=e.touches?e.touches[0].clientX:e.clientX;var cy=e.touches?e.touches[0].clientY:e.clientY;var nx=Math.max(0,Math.min(100,((cx-r.left)/r.width)*100-drOff.x));var ny=Math.max(0,Math.min(100,((cy-r.top)/r.height)*100-drOff.y));saveTk(allT.map(function(t){return t.id===drId?Object.assign({},t,{x:nx,y:ny}):t}))}
...
return <div style={{display:"flex",flexDirection:"column",gap:6}}><div style={{fontFamily:"'Cinzel',serif",fontWeight:700,fontSize:13,textAlign:"center"}}>🗺️ Карта</div><div style={{position:"relative",width:"100%",borderRadius:10,overflow:"hidden",border:"2px solid #e8e0d4",touchAction:"none",userSelect:"none"}} onMouseMove={onMv} onMouseUp={endDr} onMouseLeave={endDr} onTouchMove={onMv} onTouchEnd={endDr}><img src={img} style={{width:"100%",display:"block",pointerEvents:"none"}} alt="map"/>{allT.map(function(t){var sz=t.size||14;return <div key={t.id} onMouseDown={function(e){startDr(e,t)}} onTouchStart={function(e){startDr(e,t)}} style={{position:"absolute",left:t.x+"%",top:t.y+"%",width:sz,height:sz,borderRadius:"50%",background:t.color||"#3b82f6",border:"2px solid #fff",boxShadow:drId===t.id?"0 0 8px rgba(0,0,0,0.5)":"0 1px 3px rgba(0,0,0,0.3)",transform:"translate(-50%,-50%)",cursor:canDr(t)?"grab":"default",zIndex:drId===t.id?50:10}} title={t.name}/>})}</div>
<div style={{display:"flex",flexWrap:"wrap",gap:3}}>{allT.map(function(t){return <div key={t.id} style={{display:"flex",alignItems:"center",gap:3,background:"#fefdfb",border:"1px solid #e8e0d4",borderRadius:5,padding:"2px 5px",fontSize:8}}><div style={{width:8,height:8,borderRadius:"50%",background:t.color||"#3b82f6"}}/><span style={{fontWeight:600}}>{t.name}</span>{(isGM||t.charId===cId)&&<span style={{display:"flex",gap:1}}><button onClick={function(){saveTk(allT.map(function(x){return x.id===t.id?Object.assign({},x,{size:Math.max(6,(x.size||14)-2)}):x}))}} style={{background:"none",border:"none",cursor:"pointer",fontSize:8,color:"#8b7e6a"}}>−</button><button onClick={function(){saveTk(allT.map(function(x){return x.id===t.id?Object.assign({},x,{size:Math.min(40,(x.size||14)+2)}):x}))}} style={{background:"none",border:"none",cursor:"pointer",fontSize:8,color:"#8b7e6a"}}>+</button></span>}{isGM&&<button onClick={function(){saveTk(allT.filter(function(x){return x.id!==t.id}))}} style={{background:"none",border:"none",color:"#ef4444",cursor:"pointer",fontSize:8}}>✕</button>}</div>})}</div>
{!isGM&&cId&&!allT.find(function(t){return t.charId===cId})&&<button onClick={function(){var me=chars.find(function(c2){return(c2._fbId||c2.id)===cId});saveTk(allT.concat([{id:"ch-"+cId,charId:cId,name:me?me.name:"?",x:50,y:50,color:"#3b82f6",size:14,type:"player"}]))}} style={{padding:"6px 12px",borderRadius:7,border:"2px solid #3b82f630",background:"#eff6ff",fontWeight:700,fontSize:10,color:"#1d4ed8",cursor:"pointer",width:"100%"}}>📍 Создать мой жетон на карте</button>}
...
{isGM&&sn&&<div style={{background:"#fef2f2",border:"1px solid #ef444420",borderRadius:7,padding:6,display:"flex",flexDirection:"column",gap:3}}><input style={S.inp} value={nn} onChange={function(e){sNN(e.target.value)}} placeholder="Имя NPC"/><div style={{display:"flex",gap:2}}>{["#ef4444","#f59e0b","#8b5cf6","#1f2937","#10b981","#ec4899"].map(function(cl){return <button key={cl} onClick={function(){sNC(cl)}} style={{width:18,height:18,borderRadius:"50%",background:cl,border:nc===cl?"3px solid #fff":"2px solid #ddd",boxShadow:nc===cl?"0 0 0 2px "+cl:"none",cursor:"pointer"}}/>})}<input style={Object.assign({},S.inp,{width:40,fontSize:9,padding:2})} type="number" min={6} max={40} value={ns} onChange={function(e){sNS(parseInt(e.target.value)||14)}}/></div><button onClick={function(){if(!nn.trim())return;saveTk(allT.concat([{id:"npc-"+Date.now(),name:nn.trim(),x:50,y:50,color:nc,size:ns,type:"npc"}]));sNN("");sSN(false)}} style={{padding:5,borderRadius:5,border:"none",background:"#ef4444",color:"#fff",fontWeight:700,fontSize:9,cursor:"pointer"}}>Добавить</button></div>}
</div>}
...
return(<div style={{flex:1,display:"flex",flexDirection:"column"}}><RollPopup roll={rP} onClose={function(){sRP(null)}}/>
...
function npcAttack(s,w){
  ...
  var tgtName=playerTgtId?((playerChars.find(function(x){return x._fbId===playerTgtId})||{}).name||"?"):"";
...
if(sel)return <GameView char={sel} save={function(d){pr.saveChar(sel._fbId,d)}} onBack={function(){sS(null)}} isGM={true} logs={pr.logs} addLog={pr.addLog} lore={pr.lore} mapData={pr.mapData} saveMap={pr.saveMap} characters={pr.characters} spawned={pr.spawned} saveSpawned={pr.saveSpawned}/>;
...
if(showBest)return <BestiaryEditor npcTempl={pr.npcTempl} saveNpcTempl={pr.saveNpcTempl} spawned={pr.spawned} saveSpawned={pr.saveSpawned} onBack={function(){sShowBest(false)}} addLog={pr.addLog} characters={pr.characters} roomCode={pr.roomCode}/>;
...
useEffect(function(){if(!room)return;sLoaded(false);var u=[];u.push(onValue(ref(db,"rooms/"+room+"/characters"),function(s){sCh(s.val()||{});sLoaded(true)}));u.push(onValue(ref(db,"rooms/"+room+"/lore"),function(s){sLo(s.val()||{})}));u.push(onValue(ref(db,"rooms/"+room+"/mapData"),function(s){sMapData(s.val()||{})}));u.push(onValue(ref(db,"rooms/"+room+"/npcTemplates"),function(s){sNpcTempl(s.val()||{})}));u.push(onValue(ref(db,"rooms/"+room+"/spawned"),function(s){sSpawned(s.val()||{})}));u.push(onValue(ref(db,"rooms/"+room+"/logs"),function(s){var d=s.val()||{};sLg(Object.values(d).sort(function(a,b){return(b.ts||0)-(a.ts||0)}).slice(0,100))}));return function(){u.forEach(function(x){x()})}},[room]);
