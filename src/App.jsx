...
/* ── GMAttackPanel — ГМ видит сцены где нужно нанести урон ── */
function GMAttackPanel(pr){
  var attacks=pr.attacks||{};
  var addLog=pr.addLog;var onRoll=pr.onRoll;var characters=pr.characters||[];
  var pending_dmg=Object.entries(attacks).filter(function(e){return e[1].status==="pending_dmg"&&!e[1].fromPlayer;});
  if(pending_dmg.length===0)return null;
  return(<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.65)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:997,animation:"fadeIn 0.2s"}}>
    <div style={{background:"#fefcf5",border:"3px solid #c4b8a4",borderRadius:16,padding:"16px 20px",minWidth:270,maxWidth:360,maxHeight:"80vh",overflowY:"auto",boxShadow:"0 20px 60px rgba(0,0,0,0.4)",animation:"popIn 0.3s",display:"flex",flexDirection:"column",gap:8}}>
      <div style={{fontFamily:"'Cinzel',serif",fontWeight:900,fontSize:15,textAlign:"center",marginBottom:2}}>⚔️ Нанести урон</div>
      {pending_dmg.map(function(entry){
        var id=entry[0];var atk=entry[1];
        var dodged=atk.dodgeRoll>=atk.hitRoll;
        ...
        return(<div key={id} style={{background:"#fff",borderRadius:10,padding:"10px 12px",border:"2px solid "+(dodged?"#10b98130":"#ef444430")}}>
          ...
          <div style={{display:"flex",gap:8,justifyContent:"center",marginBottom:6}}>
            ...
            <div style={{fontSize:18,alignSelf:"center",color:"#8b7e6a"}}>vs</div>
            <div style={{textAlign:"center"}}><div style={{fontSize:8,color:"#6b7280"}}>Уклонение</div><div style={{fontFamily:"'Cinzel',serif",fontWeight:900,fontSize:24,color:"#10b981"}}>{atk.dodgeRoll||0}</div></div>
          </div>
          <div style={{fontSize:9,color:"#8b7e6a",textAlign:"center",marginBottom:6}}>{atk.weaponName+" · "+atk.dmgDice+" · "+atk.dmgType+" · "+atk.zone}</div>
          {dodged
            ?<button onClick={skipDmg} style={{width:"100%",padding:6,borderRadius:7,border:"none",background:"#10b981",color:"#fff",fontWeight:700,fontSize:11,cursor:"pointer"}}>✅ Уклонился — закрыть</button>
            :<button onClick={doRollDmg} style={{width:"100%",padding:8,borderRadius:7,border:"none",background:"#ef4444",color:"#fff",fontFamily:"'Cinzel',serif",fontWeight:900,fontSize:13,cursor:"pointer"}}>💥 Бросить урон!</button>
          }
        </div>)
      })}
    </div>
