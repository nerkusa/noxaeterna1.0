...
/* ── GMAttackPanel: ГМ видит сцену NPC→Игрок (ждём уклонения / кидаем урон) ── */
function GMAttackPanel(pr){
  ...
  return(<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.65)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:997,animation:"fadeIn 0.2s"}}>
    <div style={{background:"#fefcf5",border:"3px solid #c4b8a4",borderRadius:16,padding:"16px 20px",minWidth:280,maxWidth:370,maxHeight:"82vh",overflowY:"auto",boxShadow:"0 20px 60px rgba(0,0,0,0.4)",animation:"popIn 0.3s",display:"flex",flexDirection:"column",gap:8}}>
      <div style={{fontFamily:"'Cinzel',serif",fontWeight:900,fontSize:15,textAlign:"center"}}>⚔️ Сцены боя</div>
      {active.map(function(entry){
        ...
        return(<div key={id} style={{background:"#fff",borderRadius:10,padding:"10px 12px",border:"2px solid "+(waiting?"#f59e0b40":dodged?"#10b98130":"#ef444430")}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
            <span style={{fontWeight:700,fontSize:12}}>{atk.attackerName+" → "+atk.targetName}</span>
            <span style={{fontSize:9,padding:"2px 7px",borderRadius:99,fontWeight:700,background:waiting?"#fef3c7":dodged?"#d1fae5":"#fee2e2",color:waiting?"#92400e":dodged?"#065f46":"#991b1b"}}>{waiting?"⏳ ждём уклонения":dodged?"✅ уклонился":"❌ попало"}</span>
          </div>
          <div style={{fontSize:9,color:"#8b7e6a",textAlign:"center",marginBottom:8}}>{atk.weaponName+" · "+atk.dmgDice+" "+atk.dmgType}</div>
          <div style={{display:"flex",gap:6,justifyContent:"center",alignItems:"center",marginBottom:8}}>
            <div style={{textAlign:"center",background:"#fef2f2",borderRadius:8,padding:"5px 12px"}}>
            ...
            <div style={{fontSize:15,color:"#8b7e6a",fontWeight:700}}>vs</div>
            <div style={{textAlign:"center",background:waiting?"#fefdfb":"#f0fdf4",borderRadius:8,padding:"5px 12px",border:waiting?"1px dashed #d1d5db":"none"}}>
              <div style={{fontSize:8,color:"#6b7280",marginBottom:1}}>Уклонение</div>
              {waiting
                ?<div style={{fontSize:12,color:"#9ca3af",fontStyle:"italic",padding:"7px 4px"}}>ждём...</div>
                :<div style={{fontFamily:"'Cinzel',serif",fontWeight:900,fontSize:26,color:"#10b981"}}>{atk.dodgeRoll||0}</div>
              }
            </div>
          </div>
          {waiting&&<div style={{fontSize:9,color:"#8b7e6a",textAlign:"center",fontStyle:"italic"}}>Игрок бросает уклонение...</div>}
          {!waiting&&(dodged
            ...
            :<button onClick={doRollDmg} style={{width:"100%",padding:9,borderRadius:7,border:"none",background:"#ef4444",color:"#fff",fontFamily:"'Cinzel',serif",fontWeight:900,fontSize:14,cursor:"pointer"}}>💥 Бросить урон (авто зона)!</button>
          )}
        </div>)
      })}
    </div>
...
function PlayerAttackNotif(pr){
  ...
  return(<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.7)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:998,animation:"fadeIn 0.2s"}}>
    <div style={{background:"linear-gradient(135deg,#eff6ff,#dbeafe)",border:"3px solid #3b82f6",borderRadius:16,padding:"18px 22px",textAlign:"center",minWidth:260,maxWidth:340,boxShadow:"0 20px 60px rgba(0,0,0,0.5)",animation:"popIn 0.3s"}}>
      ...
      <button onClick={acceptHit} style={{width:"100%",padding:6,borderRadius:7,border:"2px solid #3b82f640",background:"none",color:"#3b82f6",fontWeight:700,fontSize:10,cursor:"pointer"}}>Принять удар (авто-урон)</button>
    </div>
  </div>)}
