...
/* ── NpcDeathPopup — ГМ видит что NPC погиб ── */
...
/* ── PlayerAttackNotif — ГМ видит когда игрок атакует NPC и нужно уклонение ── */
function PlayerAttackNotif(pr){
  var attacks=pr.attacks||{};
  var clearPA=pr.clearPendingAttack;
  var spawned=pr.spawned||{};
  var saveSpawned=pr.saveSpawned;
  var addLog=pr.addLog;
  var onRoll=pr.onRoll;
  /* Атаки от игроков по NPC без dodge */
  var mine=Object.entries(attacks).filter(function(e){return e[1].fromPlayer&&!e[1].dodgeRoll});
  if(mine.length===0)return null;
  var e=mine[0];var id=e[0];var atk=e[1];
  ...
  function doDodge(){
    ...
    var upd=Object.assign({},atk,{dodgeRoll:t,dodgeDetail:"d10("+d+")+DEX("+dv+")+Dodge("+dg+")="+t});
    set(ref(db,"rooms/"+pr.room+"/pendingAttacks/"+id),upd);
    addLog({who:npc.name,type:"dodge",label:"🛡️ Уклонение от "+atk.attackerName,detail:"d10("+d+")+DEX("+dv+")+Dodge("+dg+") = "+t,total:t});
    if(onRoll)onRoll({label:npc.name+" — Уклонение",d10:d,parts:[{label:"DEX",value:dv},{label:"Dodge",value:dg}],total:t,subtext:"Атака: "+atk.hitRoll+" vs Уклон: "+t+(t>=atk.hitRoll?" ✅":" ❌")});
  }
  return(<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.7)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:998,animation:"fadeIn 0.2s"}}>
    <div style={{background:"linear-gradient(135deg,#eff6ff,#dbeafe)",border:"3px solid #3b82f6",borderRadius:16,padding:"18px 22px",textAlign:"center",minWidth:260,maxWidth:340,boxShadow:"0 20px 60px rgba(0,0,0,0.5)",animation:"popIn 0.3s"}}>
      <div style={{fontSize:28,marginBottom:6}}>🎯</div>
      <div style={{fontFamily:"'Cinzel',serif",fontWeight:900,fontSize:15,color:"#1d4ed8",marginBottom:4}}>{atk.attackerName} атакует {atk.npcName}!</div>
      <div style={{background:"#fff",borderRadius:10,padding:"8px 12px",marginBottom:10}}>
        <div style={{fontSize:10,color:"#6b7280",marginBottom:2}}>Бросок на попадание</div>
        <div style={{fontFamily:"'Cinzel',serif",fontWeight:900,fontSize:32,color:"#1d4ed8"}}>{atk.hitRoll}</div>
        {atk.weaponName&&<div style={{fontSize:9,color:"#8b7e6a"}}>{atk.weaponName} • {atk.dmgType} • {atk.zone}</div>}
      </div>
      <div style={{fontSize:10,color:"#5c5548",marginBottom:12}}>Уклонение за {atk.npcName}</div>
      <button onClick={doDodge} style={{width:"100%",padding:"10px",borderRadius:9,border:"none",background:"#10b981",color:"#fff",fontFamily:"'Cinzel',serif",fontWeight:900,fontSize:14,cursor:"pointer",marginBottom:6}}>🛡️ Уклониться за NPC</button>
      <button onClick={function(){clearPA(id)}} style={{width:"100%",padding:6,borderRadius:7,border:"2px solid #3b82f640",background:"none",color:"#3b82f6",fontWeight:700,fontSize:10,cursor:"pointer"}}>Принять удар</button>
    </div>
  </div>)}
...
if(tgtNpc&&tgtId&&pr.savePendingAttack){pr.savePendingAttack({id:"atk_"+Date.now(),fromPlayer:true,attackerName:c.name||"???",npcId:tgtId,npcName:tgtNpc.name,hitRoll:t,weaponName:w.name,dmgDice:w.dmgDice||"1d6",dmgType:w.dmgType||"Р",dmgBonus:w.bonus||0,zone:selZone,ts:Date.now()});}
...
var PACKS1=[
  {icon:"💪",name:"Стартовый пак",price:"500 ₽",items:["Начните с преимущества!","+1 к силе, +1 к ловкости","+500 золота","Одно случайное зелье"]},
  {icon:"🛡️",name:"Стандартный пак",price:"1000 ₽",items:["Станьте сильнее!","+2 к любой характеристике","+1000 золота","Один редкий магический предмет"]},
  {icon:"👑",name:"VIP пак",price:"2000 ₽",items:["Доминируйте!","Отмена крит. промаха 1×/сессию","Список слабостей монстра","Секретный разговор с NPC"]},
  {icon:"🏆",name:"King пак",price:"5000 ₽",items:["Станьте королём!","Всё из VIP","+ 2 к характеристике навсегда","Идеальный спутник-легенда","Внеочередной ход"]},
];
var PACKS2=[
  {icon:"❤️",name:"Личный запас здоровья",price:"2500 ₽",items:["Начало сессии с полным HP","+50% к макс. здоровью на сессию"]},
  {icon:"🎯",name:"Игнорирование промаха",price:"500 ₽",items:["Промах → попадание","Работает 1× за раунд"]},
  {icon:"✨",name:"Полное воскрешение",price:"1000 ₽",items:["Персонаж воскресает на месте","Все вещи, полное HP, без штрафов"]},
  {icon:"📦",name:"Сундук из другого мира",price:"1500 ₽",items:["Случайный предмет от простого до легендарного","Гарантированно ломает баланс!"]},
  {icon:"📊",name:"Изменение характеристик",price:"800 ₽",items:["Перераспределить все очки","или +2 к любой характеристике навсегда"]},
  {icon:"🔮",name:"Бесплатные заклинания",price:"400 ₽",items:["Любой класс заклинания 1×/сессию","Без затрат WILL"]},
  {icon:"🌍",name:"Мастер мира",price:"1000 ₽/сессию",items:["Отменить действие игрока или ДМа","Список слабостей монстра"]},
  {icon:"💎",name:"Титан Подземелья",price:"10 000 ₽",items:["Все донаты бесплатно навсегда","Приоритет выбора заданий","Право голоса по любым спорам"]},
];
var CONDITIONS=[
...
return(<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.92)",zIndex:1000,overflowY:"auto",fontFamily:"'Nunito',sans-serif"}}>
<div style={{maxWidth:560,margin:"0 auto",padding:"16px 12px 40px"}}>

{/* Header */}
<div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
  <div style={{fontFamily:"'Cinzel',serif",fontWeight:900,fontSize:20,color:"#f59e0b"}}>💰 Донат-магазин</div>
  <button onClick={onClose} style={{background:"none",border:"1px solid #5c5548",borderRadius:6,padding:"4px 12px",color:"#e8e0d4",cursor:"pointer",fontSize:12}}>✕ Закрыть</button>
</div>

{/* Hero banner */}
<div style={{background:"linear-gradient(135deg,#1a1008,#2d1f00)",border:"2px solid #f59e0b40",borderRadius:12,padding:"14px 16px",marginBottom:16,textAlign:"center"}}>
  {img1?<img src={img1} style={{width:"100%",maxHeight:180,objectFit:"cover",borderRadius:8,marginBottom:10}} alt="banner"/>:isGM?<label style={{display:"block",marginBottom:10,padding:"20px",borderRadius:8,border:"2px dashed #f59e0b40",color:"#f59e0b",cursor:"pointer",fontSize:10}}>🖼️ ГМ: загрузить баннер<input type="file" accept="image/*" style={{display:"none"}} onChange={function(e){var f=e.target.files&&e.target.files[0];if(!f)return;var r=new FileReader();r.onload=function(ev){setDonateImg("img1",ev.target.result)};r.readAsDataURL(f)}}/></label>:null}
  <div style={{fontFamily:"'Cinzel',serif",fontWeight:900,fontSize:15,color:"#f59e0b",marginBottom:4}}>Цены для настоящих королей Мидланда!</div>
  <div style={{fontSize:10,color:"#d97706",lineHeight:1.5}}>Хотите доминировать на каждой сессии, оставив позади своих «бесплатных» друзей?<br/>Наши донаты — это ваш ключ к абсолютной власти!</div>
</div>

{/* Pack 1 — original */}
<div style={{fontFamily:"'Cinzel',serif",fontWeight:700,fontSize:12,color:"#f59e0b",marginBottom:8}}>🎁 Пакеты доната</div>
<div style={{display:"flex",flexDirection:"column",gap:6,marginBottom:16}}>
{PACKS1.map(function(p,i){return(<div key={i} style={{background:"linear-gradient(135deg,#1a1008,#221500)",border:"1px solid #f59e0b30",borderRadius:10,padding:"10px 12px",display:"flex",gap:10,alignItems:"flex-start"}}>
  <div style={{fontSize:22,flexShrink:0}}>{p.icon}</div>
  <div style={{flex:1}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
      <span style={{fontFamily:"'Cinzel',serif",fontWeight:700,fontSize:11,color:"#fef3c7"}}>{p.name}</span>
      <span style={{fontFamily:"'Cinzel',serif",fontWeight:900,fontSize:12,color:"#f59e0b",background:"#2d1f00",borderRadius:4,padding:"1px 7px"}}>{p.price}</span>
    </div>
...
{/* Pack 2 — Мидланд */}
<div style={{fontFamily:"'Cinzel',serif",fontWeight:700,fontSize:12,color:"#f59e0b",marginBottom:8}}>⚔️ Платёжные привилегии Мидланда</div>
<div style={{display:"flex",flexDirection:"column",gap:6,marginBottom:16}}>
{PACKS2.map(function(p,i){return(<div key={i} style={{background:"linear-gradient(135deg,#1a1008,#221500)",border:"1px solid #f59e0b30",borderRadius:10,padding:"10px 12px",display:"flex",gap:10,alignItems:"flex-start"}}>
  <div style={{fontSize:20,flexShrink:0}}>{p.icon}</div>
  <div style={{flex:1}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
      <span style={{fontFamily:"'Cinzel',serif",fontWeight:700,fontSize:11,color:"#fef3c7"}}>{p.name}</span>
      <span style={{fontFamily:"'Cinzel',serif",fontWeight:900,fontSize:11,color:"#f59e0b",background:"#2d1f00",borderRadius:4,padding:"1px 7px",whiteSpace:"nowrap"}}>{p.price}</span>
    </div>
    {p.items.map(function(it,j){return<div key={j} style={{fontSize:8,color:"#d97706",lineHeight:1.6}}>• {it}</div>})}
  </div>
</div>)})}
...
{/* Companions + mounts image slot */}
{img2?<img src={img2} style={{width:"100%",maxHeight:200,objectFit:"cover",borderRadius:10,marginBottom:16}} alt="companions"/>:isGM?<label style={{display:"block",marginBottom:16,padding:"20px",borderRadius:10,border:"2px dashed #f59e0b40",color:"#f59e0b",cursor:"pointer",fontSize:10,textAlign:"center"}}>🖼️ ГМ: загрузить картинку (компаньоны/скакуны)<input type="file" accept="image/*" style={{display:"none"}} onChange={function(e){var f=e.target.files&&e.target.files[0];if(!f)return;var r=new FileReader();r.onload=function(ev){setDonateImg("img2",ev.target.result)};r.readAsDataURL(f)}}/></label>:null}

{/* QR code */}
<div style={{background:"linear-gradient(135deg,#1a1008,#221500)",border:"2px solid #f59e0b40",borderRadius:12,padding:"14px",marginBottom:16,textAlign:"center"}}>
  <div style={{fontFamily:"'Cinzel',serif",fontWeight:700,fontSize:11,color:"#f59e0b",marginBottom:10}}>💳 Оплата</div>
  {qrImg?<img src={qrImg} style={{width:180,height:180,borderRadius:8,objectFit:"contain",background:"#fff",padding:4}} alt="QR"/>:<div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:8}}>
    <div style={{width:140,height:140,background:"#111",borderRadius:8,display:"flex",alignItems:"center",justifyContent:"center",border:"2px dashed #f59e0b40"}}>
      {isGM?<label style={{cursor:"pointer",textAlign:"center",padding:10}}><div style={{fontSize:24}}>📷</div><div style={{fontSize:8,color:"#f59e0b",marginTop:4}}>ГМ: загрузить QR-код</div><input type="file" accept="image/*" style={{display:"none"}} onChange={function(e){var f=e.target.files&&e.target.files[0];if(!f)return;var r=new FileReader();r.onload=function(ev){setDonateImg("qr",ev.target.result)};r.readAsDataURL(f)}}/></label>:<div style={{fontSize:30}}>📱</div>}
    </div>
    <div style={{fontSize:9,color:"#6b7280"}}>QR-код для оплаты появится здесь</div>
  </div>}
</div>

{/* Conditions */}
<div style={{background:"#0d0d0d",border:"1px solid #374151",borderRadius:10,padding:"12px 14px",marginBottom:8}}>
  <div style={{fontFamily:"'Cinzel',serif",fontWeight:700,fontSize:10,color:"#9ca3af",marginBottom:8}}>📜 Условия использования</div>
  {CONDITIONS.map(function(c,i){return<div key={i} style={{fontSize:8,color:"#6b7280",lineHeight:1.7,marginBottom:2}}>⚠️ {c}</div>})}
</div>

<div style={{textAlign:"center",fontSize:8,color:"#374151",marginTop:12}}>Nox Aeterna Донат-магазин © Все права защищены. Здоровье не продаётся. Хотя...</div>
</div>
</div>)}
...
if(showDonate)return(<div style={{fontFamily:"'Nunito',sans-serif",minHeight:"100vh",maxWidth:520,margin:"0 auto"}}><style>{CSS}</style><DonatePage onClose={function(){sShowDonate(false)}} isGM={isGM} saveMap={saveMap} mapData={mapData}/></div>);
