...
/* Координаты клика → % от натуральных размеров картинки, независимо от зума */
function getPct(e,containerEl){
  if(!containerEl)return null;
  var r=containerEl.getBoundingClientRect();
  var cx=e.touches?e.touches[0].clientX:e.clientX;
  var cy=e.touches?e.touches[0].clientY:e.clientY;
  /* Контейнер в zoom раз больше — делим на zoom чтобы получить % натурального */
  var rawPx=((cx-r.left)/zoom);var rawPy=((cy-r.top)/zoom);
  var natW=r.width/zoom;var natH=r.height/zoom;
  return{px:Math.max(0,Math.min(100,(rawPx/natW)*100)),py:Math.max(0,Math.min(100,(rawPy/natH)*100))};
}
var _mapCont=useState(null);var mapContRef=_mapCont[0];var sMapContRef=_mapCont[1];
function startDr(e,t){if(!canDr(t))return;e.preventDefault();var p=getPct(e,mapContRef);if(!p)return;sDrId(t.id);sDrOff({x:p.px-t.x,y:p.py-t.y})}
function onMv(e){if(!drId)return;e.preventDefault();var p=getPct(e,mapContRef);if(!p)return;saveTk(allT.map(function(t){return t.id===drId?Object.assign({},t,{x:Math.max(0,Math.min(100,p.px-drOff.x)),y:Math.max(0,Math.min(100,p.py-drOff.y))}):t}))}
...
<div style={{overflow:"auto",borderRadius:10,border:"2px solid #e8e0d4",background:"#1a1a2e",maxHeight:500}}>
  <div ref={function(el){sMapContRef(el)}} style={{position:"relative",display:"inline-block",touchAction:"none",userSelect:"none"}} onMouseMove={onMv} onMouseUp={endDr} onMouseLeave={endDr} onTouchMove={onMv} onTouchEnd={endDr}>
    <img src={img} style={{width:(100*zoom)+"%",minWidth:300,display:"block",pointerEvents:"none",verticalAlign:"top"}} alt="map"/>
    {allT.map(function(t){
      var sz=t.size||6;
      /* left/top в % от zoomed-размера = t.x/t.y (они % натуральных) * zoom → но т.к. контейнер уже zoom*natW, проценты сохраняются */
      return <div key={t.id} onMouseDown={function(e){startDr(e,t)}} onTouchStart={function(e){startDr(e,t)}} style={{position:"absolute",left:t.x+"%",top:t.y+"%",width:Math.max(2,sz),height:Math.max(2,sz),borderRadius:"50%",background:t.color||"#3b82f6",border:sz>=4?"1.5px solid rgba(255,255,255,0.85)":"none",boxShadow:drId===t.id?"0 0 0 2px #fff":"0 1px 3px rgba(0,0,0,0.8)",transform:"translate(-50%,-50%)",cursor:canDr(t)?"grab":"default",zIndex:drId===t.id?50:10}} title={t.name}/>
    })}
  </div>
</div>
