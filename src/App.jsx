          ...
          :<div>
            ...
            {!dodged&&atk.status==="pending_dmg"&&(function(){
              ...
              function doPlayerDmg(){
                ...
                if(onRoll)onRoll({label:"💥 Урон по "+atk.npcName,d10:null,parts:[{label:atk.dmgDice||"1d6",value:sm(dice)},{label:"Бнс",value:atk.dmgBonus||0}],total:rawDmg,subtext:zoneObj.e+" "+zoneObj.name+" ×"+zoneObj.mult+"\n"+ae.desc+"\n→ "+atk.npcName+": "+(npc.hp||0)+"→"+newNpcHp+" HP"});
                remove(ref(db,"rooms/"+pr.room+"/pendingAttacks/"+id));
              }
              return(<div style={{marginTop:8}}>
                <button onClick={doPlayerDmg} style={{width:"100%",padding:10,borderRadius:8,border:"none",background:"#ef4444",color:"#fff",fontFamily:"'Cinzel',serif",fontWeight:900,fontSize:14,cursor:"pointer"}}>💥 Бросить урон!</button>
...
function PlayerAttackNotif(pr){
  ...
  /* pending_dmg — игрок сам бросает урон, ГМ не показываем */
  var needDmg=[];
