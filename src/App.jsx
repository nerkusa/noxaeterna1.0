        ...
        {waiting
          ...
          :<div>
            ...
            {!dodged&&(function(){
              ...
              var shObj=(myChar.shields||[]).find(function(s){return s.id===shieldId});
              ...
              var canShield=shObj&&shObj.hp>0&&!is2h&&!atk.shieldUsed;
              if(!canShield)return <div style={{fontSize:10,color:"#8b7e6a",marginTop:6,fontStyle:"italic"}}>{shObj&&shObj.hp<=0?"💔 Щит сломан":"ГМ наносит урон..."}</div>;
              return <div style={{marginTop:8}}>
                <button onClick={function(){update(ref(db,"rooms/"+pr.room+"/pendingAttacks/"+id),{shieldUsed:true,shieldAbsorb:shObj.absorb,shieldName:shObj.name,shieldId:shObj.id});}} style={{width:"100%",padding:9,borderRadius:8,border:"none",background:"#0369a1",color:"#fff",fontFamily:"'Cinzel',serif",fontWeight:900,fontSize:13,cursor:"pointer",marginBottom:4}}>🛡 Выставить {shObj.name} ({shObj.absorb*100}% поглощения)</button>
                <div style={{fontSize:9,color:"#8b7e6a",textAlign:"center"}}>HP щита: {shObj.hp}/{shObj.maxHp}</div>
              </div>;
            })()}
          </div>
        }
...
function GMAttackPanel(pr){
  ...
  function doRollDmg(){
    ...
    if(atk.shieldUsed&&atk.shieldId){updP.shields=(tgtChar.shields||[]).map(function(s){return s.id===atk.shieldId?Object.assign({},s,{hp:Math.max(0,s.hp-shieldDmg)}):s;});if(updP.shields.find(function(s){return s.id===atk.shieldId&&s.hp<=0}))updP.equippedShield=null;}
...
<div style={{fontSize:8,fontWeight:700,color:"#0369a1"}}>🛡 Щит (левая рука){shieldBlocked&&<span style={{color:"#f59e0b",marginLeft:4}}>⚠️ недоступен</span>}</div>
<button onClick={function(){sSsAdd(!ssAdd)}} style={{fontSize:8,background:"none",border:"none",cursor:"pointer",color:"#0369a1",fontWeight:700}}>{ssAdd?"✕":"+ Добавить"}</button>
</div>
{ssAdd&&<div style={{background:"#e0f2fe",borderRadius:6,padding:5,marginBottom:5,display:"flex",flexDirection:"column",gap:3}}>
...
<button onClick={function(){if(!shn.trim())return;var tObj=SHIELD_T.find(function(x){return x.id===sht})||SHIELD_T[0];if(fs.BODY<tObj.bodyReq){alert("BODY("+fs.BODY+") < "+tObj.bodyReq);return;}var ns={id:Date.now(),name:shn.trim(),type:sht,absorb:tObj.absorb,hp:shhp,maxHp:shhp};sv(Object.assign({},c,{shields:(c.shields||[]).concat([ns])}));sShn("");sSsAdd(false);}} style={{padding:4,borderRadius:4,border:"none",background:"#0369a1",color:"#fff",fontWeight:700,fontSize:9,cursor:"pointer"}}>Добавить</button>
</div>}
{shields.length===0&&!ssAdd&&<div style={{fontSize:8,color:"#8b7e6a",fontStyle:"italic"}}>Нет щитов</div>}
{shields.map(function(sh){var isEqSh=c.equippedShield===sh.id;var shPct=sh.maxHp>0?(sh.hp/sh.maxHp)*100:0;var shTyp=SHIELD_T.find(function(x){return x.id===sh.type})||SHIELD_T[0];
return <div key={sh.id} style={{background:isEqSh?"#e0f2fe":"#fefdfb",border:"1px solid "+(isEqSh?"#0369a130":"#e8e0d4"),borderRadius:5,padding:"3px 5px",marginBottom:3,display:"flex",flexDirection:"column",gap:2}}>
<div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
<span style={{fontSize:9,fontWeight:isEqSh?700:400}}>{sh.name} <span style={{fontSize:7,color:"#64748b"}}>{shTyp.name+" "+shTyp.absorb*100+"%"}</span></span>
...
<span style={{fontSize:9,fontWeight:700,color:sh.hp<=0?"#ef4444":"#0369a1",minWidth:28,textAlign:"center"}}>{sh.hp+"/"+sh.maxHp}</span>
...
{!isEqSh&&<button disabled={shieldBlocked} onClick={function(){if(!shieldBlocked)sv(Object.assign({},c,{equippedShield:sh.id}));}} style={{fontSize:7,padding:"1px 5px",borderRadius:3,border:"1px solid #0369a120",background:"#e0f2fe",cursor:shieldBlocked?"not-allowed":"pointer",color:"#0c4a6e",opacity:shieldBlocked?0.5:1}}>Надеть</button>}
{isEqSh&&<button onClick={function(){sv(Object.assign({},c,{equippedShield:null}))}} style={{fontSize:7,padding:"1px 5px",borderRadius:3,border:"1px solid #ef444420",background:"#fee2e2",cursor:"pointer",color:"#ef4444"}}>Снять</button>}
{!isEqSh&&<button onClick={function(){sv(Object.assign({},c,{shields:(c.shields||[]).filter(function(x){return x.id!==sh.id})}))}} style={{background:"none",border:"none",color:"#ef4444",fontSize:9,cursor:"pointer"}}>✕</button>}
</div></div>
<div style={{background:"#fff",borderRadius:2,height:4,overflow:"hidden"}}><div style={{height:"100%",width:shPct+"%",background:sh.hp<=0?"#ef4444":"#0369a1",borderRadius:2}}/></div>
...
<button onClick={function(){sv(Object.assign({},c,{weaponMode:"1h"}))}} style={{flex:1,padding:"2px 0",borderRadius:4,border:curMode==="1h"?"2px solid #3b82f6":"1px solid #e8e0d4",background:curMode==="1h"?"#eff6ff":"#fefdfb",fontSize:8,fontWeight:curMode==="1h"?700:400,cursor:"pointer",color:curMode==="1h"?"#1d4ed8":"#5c5548"}}>1 рука · {w.dmgDice}{w.bonus?" +"+w.bonus:""}</button>
