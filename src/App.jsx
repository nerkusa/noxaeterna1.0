...
function PlayerAttackStatus(pr){
  ...
  var dodged=!waiting&&!shieldPhase&&(atk.dodgeRoll||0)>=(atk.hitRoll||0);
...
function PendingAttackPopup(pr){
  ...
  var pending=Object.entries(attacks).filter(function(e){
    return e[1].targetId===myId&&(e[1].status==="pending_dodge"||e[1].status==="pending_shield"||e[1].status==="pending_dmg"||!e[1].status);
  });
  ...
  function doDodge(){
    ...
    update(ref(db,"rooms/"+pr.room+"/pendingAttacks/"+id),{dodgeRoll:t,dodgeDetail:"d10("+d+")+DEX("+dv+")+Dodge("+dg+")="+t,status:dodgedNow?"done":"pending_shield"});
  ...
  return(<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.75)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:998,animation:"fadeIn 0.2s"}}>
    <div style={{background:"#fefcf5",border:"3px solid "+(waiting?"#ef4444":dodged?"#10b981":"#ef4444"),borderRadius:16,padding:"18px 22px",textAlign:"center",minWidth:270,maxWidth:350,boxShadow:"0 20px 60px rgba(0,0,0,0.5)",animation:"popIn 0.3s"}}>
      ...
      <div style={{background:"#fff",border:"1px solid "+(waiting?"#d1d5db":shieldPhaseGM?"#0369a140":"#10b98140"),borderRadius:10,padding:"8px 12px",marginBottom:waiting?10:0}}>
          ...
          :<div>
            ...
            {shieldPhase&&(function(){
              ...
              return(<div style={{marginTop:8,display:"flex",flexDirection:"column",gap:6}}>
                ...
                <button onClick={function(){update(ref(db,"rooms/"+pr.room+"/pendingAttacks/"+id),{status:"pending_dmg",shieldUsed:false});}} style={{width:"100%",padding:8,borderRadius:8,border:"2px solid #ef444440",background:"none",color:"#ef4444",fontWeight:700,fontSize:11,cursor:"pointer"}}>💥 Принять удар без щита</button>
              </div>);
            })()}
...
function GMAttackPanel(pr){
  ...
  var active=Object.entries(attacks).filter(function(e){
    return !e[1].fromPlayer&&(e[1].status==="pending_dodge"||e[1].status==="pending_shield"||e[1].status==="pending_dmg"||(!e[1].status&&!e[1].fromPlayer&&e[1].targetId));
  });
  if(active.length===0)return null;
  var entry=active[0];var id=entry[0];var atk=entry[1];
  var waiting=atk.status==="pending_dodge";var shieldPhaseGM=atk.status==="pending_shield";
  var dodged=!waiting&&!shieldPhaseGM&&(atk.dodgeRoll||0)>=(atk.hitRoll||0);
  ...
  return(<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.72)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:997,animation:"fadeIn 0.2s"}}>
    <div style={{background:"#fefcf5",border:"3px solid "+(waiting?"#f59e0b":dodged?"#10b981":shieldPhaseGM?"#0369a1":"#ef4444"),borderRadius:16,padding:"18px 22px",textAlign:"center",minWidth:270,maxWidth:350,boxShadow:"0 20px 60px rgba(0,0,0,0.5)",animation:"popIn 0.3s"}}>
      ...
      <div style={{background:"#fff",border:"1px solid "+(waiting?"#d1d5db":shieldPhaseGM?"#0369a140":"#10b98140"),borderRadius:10,padding:"8px 12px",marginBottom:10}}>
        ...
        {waiting
          ...
          :shieldPhaseGM
          ?<div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:8,padding:"6px 0"}}>
            <div style={{width:8,height:8,borderRadius:"50%",background:"#0369a1",animation:"fadeIn 0.5s infinite alternate"}}/>
            <span style={{fontSize:12,color:"#0369a1",fontStyle:"italic"}}>игрок решает — щит или нет...</span>
          </div>
          :<div>
      ...
      {/* Кнопки */}
      {(waiting||shieldPhaseGM)&&<div style={{fontSize:9,color:"#8b7e6a",fontStyle:"italic"}}>{shieldPhaseGM?"⏳ Игрок выбирает защиту...": "Ожидаем действия игрока..."}</div>}
