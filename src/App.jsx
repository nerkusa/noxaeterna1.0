...
<div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
  ...
  <div style={{display:"flex",alignItems:"center",gap:4}}>
    <button onClick={function(){sZoom(function(z){return Math.max(0.5,+(z-0.25).toFixed(2))})}} style={{width:24,height:24,borderRadius:5,border:"1px solid #e8e0d4",background:"#fefdfb",fontWeight:700,fontSize:13,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>−</button>
    <span style={{fontSize:9,fontWeight:700,minWidth:32,textAlign:"center"}}>{Math.round(zoom*100)+"%"}</span>
    <button onClick={function(){sZoom(function(z){return Math.min(4,+(z+0.25).toFixed(2))})}} style={{width:24,height:24,borderRadius:5,border:"1px solid #e8e0d4",background:"#fefdfb",fontWeight:700,fontSize:13,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>+</button>
...
<div style={{overflow:"auto",borderRadius:10,border:"2px solid #e8e0d4",background:"#1a1a2e",maxHeight:480}}>
  <div style={{position:"relative",display:"inline-block",touchAction:"none",userSelect:"none",minWidth:"100%"}} onMouseMove={onMv} onMouseUp={endDr} onMouseLeave={endDr} onTouchMove={onMv} onTouchEnd={endDr}>
    <img src={img} style={{width:(100*zoom)+"%",minWidth:"100%",display:"block",pointerEvents:"none"}} alt="map"/>
    {allT.map(function(t){var sz=t.size||6;return <div key={t.id} onMouseDown={function(e){startDr(e,t)}} onTouchStart={function(e){startDr(e,t)}} style={{position:"absolute",left:t.x+"%",top:t.y+"%",width:sz*zoom,height:sz*zoom,borderRadius:"50%",background:t.color||"#3b82f6",border:sz*zoom>=6?"1.5px solid rgba(255,255,255,0.9)":"none",boxShadow:drId===t.id?"0 0 6px rgba(255,255,255,0.8)":"0 1px 2px rgba(0,0,0,0.6)",transform:"translate(-50%,-50%)",cursor:canDr(t)?"grab":"default",zIndex:drId===t.id?50:10,flexShrink:0}} title={t.name}/>})}
  </div>
</div>
