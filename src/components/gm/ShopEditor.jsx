...
function genId() { return 'sh_' + Date.now().toString(36) + Math.floor(Math.random() * 1000).toString(36); }

function field(label, node) {
  return <div style={{ flex: 1 }}><label style={lbl}>{label}</label>{node}</div>;
}
...
export default function ShopEditor(pr) {
  ...
  const add = function () {
    ...
    const it = Object.assign({ id: genId() }, base);
    ...
    if (it.cat === 'weapon') {
      return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginTop: 6 }}>
          {field('Название', <input value={it.name} onChange={function (e) { upd(it.id, { name: e.target.value }); }} style={inp} />)}
          <div style={{ display: 'flex', gap: 6 }}>
            {field('Тип', <select value={it.wtype} onChange={function (e) { upd(it.id, { wtype: e.target.value }); }} style={Object.assign({}, inp, { cursor: 'pointer' })}>{WT.map(function (t) { return <option key={t} value={t}>{t}</option>; })}</select>)}
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
      ...
      <div style={{ display: 'flex', gap: 4 }}>
        {CATS.map(function (cc) {
          const on = cat === cc.id;
          return <button key={cc.id} onClick={function () { setCat(cc.id); setEditId(null); }} style={{ flex: 1, padding: '6px 2px', borderRadius: 7, border: '2px solid ' + cc.color + (on ? '' : '20'), background: on ? cc.color + '20' : '#1d1a14', color: cc.color, fontWeight: 700, fontSize: 10, cursor: 'pointer' }}>{cc.name}</button>;
        })}
      </div>
