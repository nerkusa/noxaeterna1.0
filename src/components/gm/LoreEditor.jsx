...
if(ch&&ch.id==="map")return <div style={{display:"flex",flexDirection:"column",gap:8}}><button onClick={function(){sE(null)}} style={{alignSelf:"flex-start",padding:"4px 10px",borderRadius:6,border:"2px solid #322d24",background:"#1d1a14",fontWeight:700,fontSize:10,cursor:"pointer"}}>← Назад</button><MapView mapData={pr.mapData} saveMap={pr.saveMap} characters={pr.characters} isGM={true}/></div>;
if(ch){return(<div style={{display:"flex",flexDirection:"column",gap:8}}><button onClick={function(){sE(null)}} style={{alignSelf:"flex-start",padding:"4px 10px",borderRadius:6,border:"2px solid #322d24",background:"#1d1a14",fontWeight:700,fontSize:10,cursor:"pointer"}}>← Назад</button><div style={{display:"flex",alignItems:"center",gap:6}}><span style={{fontSize:20}}>{ch.icon}</span><span style={{fontFamily:"'Cinzel',serif",fontWeight:900,fontSize:14,color:ch.color}}>{ch.title}</span></div><textarea style={Object.assign({},S.inp,{minHeight:300,resize:"vertical",fontSize:10,padding:8,lineHeight:1.5})} value={lore[ch.id]||""} onChange={function(e){var n=Object.assign({},lore);n[ch.id]=e.target.value;sLore(n)}}/></div>)}
