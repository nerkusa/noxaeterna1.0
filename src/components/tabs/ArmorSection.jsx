...
{(function(){
  ...
  var doLog=function(amt,what){if(pr.addLog)pr.addLog({who:c.name||"???",type:"rest",label:"🔧 Починка: "+what+" +"+amt+" HP",detail:die+(isArt?"+CRA("+(fs.CRA||0)+")":""),total:amt});if(pr.onRoll)pr.onRoll({label:"🔧 Починка брони",d10:null,parts:[],total:amt,subtext:what+": +"+amt+" HP\n"+pwr});};
  ...
  return <div style={{marginTop:6,padding:"5px 7px",background:"#231b08",border:"1px solid #f59e0b30",borderRadius:7}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><span style={{fontSize:8,fontWeight:700,color:"#f0b352"}}>🔧 Ремонт брони (1/день)</span><span style={{fontSize:7,color:"#a89a82"}}>{used?"✓ использовано":canRepair?pwr:"нужны инструменты"}</span></div>
