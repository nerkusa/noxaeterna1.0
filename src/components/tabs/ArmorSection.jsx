...
{(function(){
  ...
  var hasTools=(c.inventory||[]).some(function(i){return /инструмент|tool|набор|kit/i.test(i.name||"")});
  var used=!!c.repairUsed;
  var canRepair=isArt||hasTools;
  var rollAmt=function(){return isArt?(r1(8)+(fs.CRA||0)):r1(4)};
  var doLog=function(amt,what){if(pr.addLog)pr.addLog({who:c.name||"???",type:"rest",label:"🔧 Починка: "+what+" +"+amt+" HP",detail:isArt?("1d8+CRA("+(fs.CRA||0)+")"):"1d4",total:amt});if(pr.onRoll)pr.onRoll({label:"🔧 Починка брони",d10:null,parts:[],total:amt,subtext:what+": +"+amt+" HP\n"+(isArt?"1d8 + CRA":"1d4")});};
  var repOwn=function(kind,id,nm){if(used||!canRepair)return;var amt=rollAmt();var patch={repairUsed:true};if(kind==="armor")patch.armors=(c.armors||[]).map(function(a){return a.id===id?Object.assign({},a,{hp:Math.min(a.maxHp,(a.hp||0)+amt)}):a});else patch.shields=(c.shields||[]).map(function(x){return x.id===id?Object.assign({},x,{hp:Math.min(x.maxHp,(x.hp||0)+amt)}):x});sv(Object.assign({},c,patch));doLog(amt,nm);};
  var repAlly=function(al,kind,id,nm){if(used||!isArt||!pr.room)return;var amt=rollAmt();var upd=Object.assign({},al);delete upd._fbId;if(kind==="armor")upd.armors=(al.armors||[]).map(function(a){return a.id===id?Object.assign({},a,{hp:Math.min(a.maxHp,(a.hp||0)+amt)}):a});else upd.shields=(al.shields||[]).map(function(x){return x.id===id?Object.assign({},x,{hp:Math.min(x.maxHp,(x.hp||0)+amt)}):x});set(ref(db,"rooms/"+pr.room+"/characters/"+al._fbId),upd);sv(Object.assign({},c,{repairUsed:true}));doLog(amt,al.name+" · "+nm);};
  var dmgOf=function(o){return (o.armors||[]).filter(function(a){return (a.hp||0)<a.maxHp}).map(function(a){return {kind:"armor",id:a.id,nm:a.name,hp:a.hp,mhp:a.maxHp}}).concat((o.shields||[]).filter(function(x){return (x.hp||0)<x.maxHp}).map(function(x){return {kind:"shield",id:x.id,nm:x.name,hp:x.hp,mhp:x.maxHp}}))};
  var ownDmg=dmgOf(c);
  ...
  return <div style={{marginTop:6,padding:"5px 7px",background:"#231b08",border:"1px solid #f59e0b30",borderRadius:7}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><span style={{fontSize:8,fontWeight:700,color:"#f0b352"}}>🔧 Ремонт брони (1/день)</span><span style={{fontSize:7,color:"#a89a82"}}>{used?"✓ использовано":canRepair?(isArt?"1d8+CRA":"1d4"):"нужны инструменты"}</span></div>
    ...
    {!used&&!canRepair&&<div style={{fontSize:7,color:"#a89a82",marginTop:2,fontStyle:"italic"}}>Нужны «инструменты» в инвентаре (или класс Ремесленник)</div>}
