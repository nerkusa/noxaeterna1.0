...
{(wdt==="П"||wt==="Archery")&&<div style={{display:"flex",gap:3,alignItems:"center"}}><span style={{fontSize:8,color:"#f0b352",width:60}}>{wt==="Archery"?"🏹 Колчан:":"🔫 Обойма:"}</span><input style={Object.assign({},S.inp,{width:50,fontSize:9,padding:3})} type="number" value={wcl} onChange={function(e){sWCL(parseInt(e.target.value)||1)}} placeholder="1"/></div>}
<button onClick={function(){if(!wn.trim())return;var newW={id:uid(),name:wn.trim(),type:wt,dmgType:wdt,bonus:wb,dmgDice:wdi,hands:wh};if(wh===1.5){newW.dmgDice2h=wdi2;newW.bonus2h=wb2;}if(wdt==="П"||wt==="Archery"){newW.clip=wcl;newW.ammo=wcl;}sv(Object.assign({},c,{weapons:(c.weapons||[]).concat([newW])}));sWN("");sSA(false);sWH(1);sWDI("1d6");sWDI2("2d6");sWB2(0);sWCL(1);}} style={{padding:5,borderRadius:5,border:"none",background:"#10b981",color:"#fff",fontWeight:700,fontSize:10,cursor:"pointer"}}>Добавить</button>
</div>}
