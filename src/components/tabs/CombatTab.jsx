...
var _wcl=useState(6);var wcl=_wcl[0];var sWCL=_wcl[1];
/* weapon edit state */
...
{wdt==="П"&&<div style={{display:"flex",gap:3,alignItems:"center"}}><span style={{fontSize:8,color:"#f0b352",width:60}}>🔫 Обойма:</span><input style={Object.assign({},S.inp,{width:50,fontSize:9,padding:3})} type="number" value={wcl} onChange={function(e){sWCL(parseInt(e.target.value)||1)}} placeholder="6"/></div>}
<button onClick={function(){if(!wn.trim())return;var newW={id:uid(),name:wn.trim(),type:wt,dmgType:wdt,bonus:wb,dmgDice:wdi,hands:wh};if(wh===1.5){newW.dmgDice2h=wdi2;newW.bonus2h=wb2;}if(wdt==="П"){newW.clip=wcl;newW.ammo=wcl;}sv(Object.assign({},c,{weapons:(c.weapons||[]).concat([newW])}));sWN("");sSA(false);sWH(1);sWDI("1d6");sWDI2("2d6");sWB2(0);sWCL(6);}} style={{padding:5,borderRadius:5,border:"none",background:"#10b981",color:"#fff",fontWeight:700,fontSize:10,cursor:"pointer"}}>Добавить</button>
</div>}
...
{(c.weapons||[]).map(function(w,wIdx){var sk=WS[w.type]||"Simple Weapon";var statKey=wStat(w.type);var isEq=c.equippedWeapon===w.id;var isGun=w.dmgType==="П";var clip=w.clip||(isGun?6:0);var ammo=(w.ammo!==undefined&&w.ammo!==null)?w.ammo:clip;var curMode=c.weaponMode||"1h";var activeDice=(w.hands===1.5&&curMode==="2h")?(w.dmgDice2h||w.dmgDice):w.dmgDice;var activeBon=(w.hands===1.5&&curMode==="2h")?(w.bonus2h!==undefined?w.bonus2h:w.bonus||0):(w.bonus||0);var handsLabel=w.hands===2?"двуручное":w.hands===1.5?"полуторное":"одноручное";var handsClr=w.hands===2?"#f87171":w.hands===1.5?"#f0b352":"#60a5fa";var handsBg=w.hands===2?"#311717":w.hands===1.5?"#231b08":"#0e1a2b";
...
<span style={{fontSize:9,fontWeight:700,color:ammo<=0?"#ef4444":"#f0b352"}}>{"🔫 "+ammo+"/"+clip}</span>
<button onClick={function(){sv(Object.assign({},c,{weapons:(c.weapons||[]).map(function(x){return x.id===w.id?Object.assign({},x,{clip:clip,ammo:clip}):x})}));if(pr.addLog)pr.addLog({who:c.name||"???",type:"rest",label:"🔄 "+w.name+" — перезарядка ("+clip+")",detail:"",total:0})}} style={{padding:"2px 8px",borderRadius:5,border:"1px solid #f59e0b40",background:"#231b08",fontSize:8,fontWeight:700,color:"#f0b352",cursor:"pointer"}}>🔄 Перезарядить</button>
{ammo<=0&&<span style={{fontSize:8,color:"#ef4444",fontWeight:700}}>пусто!</span>}
</div>}
<div style={{display:"flex",gap:3}}>
<button onClick={function(){if(isGun&&ammo<=0){alert("🔫 Нет патронов — перезаряди!");return}var R=rollHit();var d=R.d;var rv=fs[statKey]||0;var sv2=es[sk]||0;var warBon=(c.warriorBonus&&(pf.id==="warrior"||pf.abilityType==="bonus_attack"))?5:0;var _patch={};if(warBon)_patch.warriorBonus=false;if(isGun)_patch.weapons=(c.weapons||[]).map(function(x){return x.id===w.id?Object.assign({},x,{clip:clip,ammo:Math.max(0,ammo-1)}):x});if(Object.keys(_patch).length)sv(Object.assign({},c,_patch));var aimP=(aim&&tgtNpc)?aimPen(selZone):0;var t=d+rv+sv2+(w.bonus||0)+warBon-aimP;pr.addLog({who:c.name||"???",type:"hit",label:"🎯 "+w.name+(tgtNpc?" → "+tgtNpc.name:"")+(aimP?" 🎯"+selZone+"(−"+aimP+")":"")+(warBon?" ⚔️+5":"")+(R.crit?" 🌟КРИТ":R.fumble?" 💀ПРОВАЛ":""),detail:"🎲"+d+" + "+statKey+"("+rv+") + "+sk+"("+sv2+") + бонус("+(w.bonus||0)+")"+(aimP?" − прицел("+aimP+")":"")+" = "+t,total:t});
